...
### 🏝️ Dawno, dawno temu... (no dobra, nie aż tak!)
...
## 🛠️ Jak? Przejdźmy do mięsa! 🍖

Czyli to, co Luffy i deweloperzy kochają najbardziej. Żeby zacząć zabawę z ananasem, musimy go najpierw dorzucić do naszego koszyka (czyt. `package.json`):
...
Teraz musimy go podpiąć tam, gdzie tworzymy naszą aplikację i montujemy root element. 

1. **Tworzymy instancję:** Wywołujemy funkcję `createPinia()` (importujemy ją prosto z 'pinia').
2. **Łączymy siły:** Gdy mamy już naszą aplikację `const app = createApp(App)`, musimy zamontować w niej Pinię.
3. **Kolejność ma znaczenie:** Robimy `app.use(pinia)` **zanim** odpalimy `app.mount('#app')`. Chcemy przecież, żeby nasz ananas już działał i był gotowy do akcji, zanim reszta aplikacji wskoczy na scenę!
...
app.mount('#app') // ⚓ Łapiemy id z index.html i montujemy
```

**WoW!!! Czy to takie proste?** Otóż to! Pierwszy etap za nami. Brawo! 👏👏👏

Skoro mamy już Pinię zainstalowaną, to czas stworzyć nasz pierwszy **Store**. WuuuHuuuu!!! 

W świecie Pinii mamy dwa sposoby na pisanie "zaklęć":
1. **Option Store** – bardziej klasyczny, przypominający Options API (data, methods itd.), czyli obiekt z kluczami i wartościami.
2. **Setup Store** – nowocześniejszy, korzystający z dobrodziejstw Composition API (refy, computed, funkcje), czyli na prędkości piszemy sobie kod co chcemy robić, tylko musimy pamiętać by zwrócić później obiekt z tym co użyliśmy ;)
